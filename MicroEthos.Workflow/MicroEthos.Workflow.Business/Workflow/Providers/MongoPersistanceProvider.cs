...
public class MongoPersistenceProvider : IPersistenceProvider
{
    ...
    private IMongoCollection<WorkflowInstance> WorkflowInstances =>
        _database.GetCollection<WorkflowInstance>(WorkflowCollectionName);
    ...
    private IMongoCollection<ScheduledCommand> ScheduledCommands =>
        _database.GetCollection<ScheduledCommand>("wfc.scheduled_commands");
    ...
    {
        var result = WorkflowInstances.AsQueryable();
        ...
        DateTime expiry, CancellationToken cancellationToken = default)
    {
        ...
        var result = await EventSubscriptions.UpdateOneAsync(
            x => x.Id == eventSubscriptionId && x.ExternalToken == null, update, cancellationToken: cancellationToken);
        ...
        while (await cursor.MoveNextAsync(cancellationToken))
            foreach (var command in cursor.Current)
                try
                {
                    ...
                    await ScheduledCommands.DeleteOneAsync(x =>
                        x.CommandName == command.CommandName && x.Data == command.Data);
                }
                catch (Exception)
                {
                    //TODO: add logger
