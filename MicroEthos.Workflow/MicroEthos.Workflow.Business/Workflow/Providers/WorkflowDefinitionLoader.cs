...
public class WorkflowDefinitionLoader : IDefinitionLoader
{
    private readonly IWorkflowRegistry _registry;

    public WorkflowDefinitionLoader(IWorkflowRegistry registry)
    {
        _registry = registry;
    }
    ...
    private WorkflowStepCollection ConvertSteps(ICollection<StepSourceV1> source, Type dataType)
    {
        ...
        while (stack.Count > 0)
        {
            ...
            {
                var cancelExprType =
                    typeof(Expression<>).MakeGenericType(typeof(Func<,>).MakeGenericType(dataType, typeof(bool)));
    ...
    private void AttachInputs(StepSourceV1 source, Type dataType, Type stepType, WorkflowStep step)
    {
        foreach (var input in source.Inputs)
        {
            ...
            {
                var acn = BuildScalarInputAction(input, dataParameter, contextParameter, environmentVarsParameter,
                    stepProperty);
            ...
            {
                var acn = BuildObjectInputAction(input, dataParameter, contextParameter, environmentVarsParameter,
                    stepProperty);
                step.Inputs.Add(new ActionParameter<IStepBody, object>(acn));
                continue;
            }
    ...
    private void AttachDirectlyOutput(KeyValuePair<string, string> output, WorkflowStep step, Type dataType,
        LambdaExpression sourceExpr, ParameterExpression dataParameter)
    {
        ...
        else
        {
            ...
            {
                var resolvedValue = sourceExpr.Compile().DynamicInvoke(pStep);
                ;
    ...
    private void AttachNestedOutput(KeyValuePair<string, string> output, WorkflowStep step, StepSourceV1 source,
        LambdaExpression sourceExpr, ParameterExpression dataParameter)
    {
        ...
        var hasAddOutput = false;

        foreach (var propertyName in paths)
        {
            if (hasAddOutput) throw new ArgumentException($"Unknown property for output {output.Key} on {source.Id}");
            ...
            if (propertyName.Contains("["))
            {
                ...
                var memberExpression = Expression.Property(targetProperty, items[0]);
                ...
                Action<IStepBody, object> acn = (pStep, pData) =>
                {
                    ...
                    var data = targetExpr.Compile().DynamicInvoke(pData);
                    var resolvedValue = sourceExpr.Compile().DynamicInvoke(pStep);
                    ;
    ...
    private void AttachOutcomes(StepSourceV1 source, Type dataType, WorkflowStep step)
    {
        ...
        {
            var sourceDelegate = DynamicExpressionParser
                .ParseLambda(new[] {dataParameter, outcomeParameter}, typeof(object), nextStep.Value).Compile();
            Expression<Func<object, object, bool>> sourceExpr = (data, outcome) =>
    ...
    {
        var sections = name.Split(',').Select(s => s.Trim()).ToArray();
        var assembly = AppDomain.CurrentDomain.GetAssemblies()
            .First(a => a.GetName().Name.Equals(sections[1], StringComparison.OrdinalIgnoreCase));
        var type = assembly.GetTypes().First(t => t.FullName.Equals(sections[0], StringComparison.OrdinalIgnoreCase));
        return type;
    }

    private static Action<IStepBody, object, IStepExecutionContext> BuildScalarInputAction(
        KeyValuePair<string, object> input, ParameterExpression dataParameter, ParameterExpression contextParameter,
        ParameterExpression environmentVarsParameter, PropertyInfo stepProperty)
    {
        var expr = System.Convert.ToString(input.Value);
        var sourceExpr =
            DynamicExpressionParser.ParseLambda(new[] {dataParameter, contextParameter, environmentVarsParameter},
                typeof(object), expr);

        void acn(IStepBody pStep, object pData, IStepExecutionContext pContext)
        {
            var resolvedValue = sourceExpr.Compile()
                .DynamicInvoke(pData, pContext, Environment.GetEnvironmentVariables());
    ...
    private static Action<IStepBody, object, IStepExecutionContext> BuildObjectInputAction(
        KeyValuePair<string, object> input, ParameterExpression dataParameter, ParameterExpression contextParameter,
        ParameterExpression environmentVarsParameter, PropertyInfo stepProperty)
    {
        void acn(IStepBody pStep, object pData, IStepExecutionContext pContext)
        {
            ...
            while (stack.Count > 0)
            {
                ...
                foreach (var prop in subobj.Properties().ToList())
                    ...
                    {
                        var sourceExpr = DynamicExpressionParser.ParseLambda(
                            new[] {dataParameter, contextParameter, environmentVarsParameter}, typeof(object),
                            prop.Value.ToString());
                        var resolvedValue = sourceExpr.Compile()
                            .DynamicInvoke(pData, pContext, Environment.GetEnvironmentVariables());
