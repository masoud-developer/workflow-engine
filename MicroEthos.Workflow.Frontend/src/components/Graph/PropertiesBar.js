...
export default class PropertiesBar extends Component {
    constructor(props) {
        ...
        if(!props.initialNode || !props.initialNode._id)
            this.state = {
                nodes: [],
                parameters: [],
                outputs: [],
    ...
    getStateDict(nodes) {
        let parameters = nodes.length > 0 ? (nodes[0].parameters || []) : []
        const commonParameters = JSON.parse(JSON.stringify(parameters));
        let ii,
            jj;
    ...
    renderOutputs(outputs) {
        return outputs.filter(
            (output) => {
                return output.values && output.values.length > 0;
            }
        ).map(
    ...
    render() {
        ...
        // let logsList = null
        // if(this.state.logs)
        //     logsList = this.renderOutputs(this.state.logs);
