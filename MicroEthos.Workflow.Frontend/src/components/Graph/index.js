...
`;
  ...
  loadGraphFromJson(data) {
    ...
    document.title = this.graph_node.title + " - Graph - Workflow";
  ...
  onNewConnector(from_nid, from_pin, to_nid, to_pin) {
    ...
    const node_output = from_node.Outputs[Object.keys(from_node.Outputs).find(s => s == from_pin)];

    const to_node = this.node_lookup[to_nid];
    const node_input = to_node.Inputs[Object.keys(to_node.Inputs).find((node_input_) => node_input_ === to_pin)];
  ...
  handleDrop(blockObjArg, replaceOriginalNode) {
    ...
    if (node.Inputs) {
      forIn(node.Inputs, (value, key) => {
        inputs.push({
          name: key,
          file_type: node.Inputs[key].type,
          is_array: node.Inputs[key].is_array,
        });
        node.Inputs[key].values = []; // clear inputs on paste
        node.Inputs[key].input_references = [];
      })
    }
    forIn(node.Outputs, (value, key) => {
      outputs.push({
        name: key,
        file_type: node.Outputs[key].type,
        is_array: node.Outputs[key].is_array,
      });
      if (node.node_running_status !== NODE_RUNNING_STATUS.STATIC) {
        node.Outputs[key].values = []; // clear outputs
      }
    });
    if(node.logs)
