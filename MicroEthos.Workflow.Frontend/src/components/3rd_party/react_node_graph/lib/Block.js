...
class Block extends React.Component {
  ...
  render() {
    const lowered = this.props.node.node_running_status?.toLowerCase() || '';
    const runningLowered = this.props.node.node_status?.toLowerCase() || '';
    const blockClass = [
      'node',
      this.state.selected ? 'selected' : '',
      `running-status-${lowered}`,
      `status-${runningLowered}`,
            ...
            <section className={blockClass} style={{zIndex: 10000}} >
                <header className="node-header">
                  <span className="node-header-title">
                      <Icon
                        type_descriptor={{icon: 'workflow.c-sharp'}}
