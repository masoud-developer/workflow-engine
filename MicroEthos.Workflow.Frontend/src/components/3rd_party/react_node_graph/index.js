...
class ReactBlockGraph extends React.Component {
  ...
  handleCompleteConnector(nid, inputIndex) {
    if (this.state.dragging) {
      ...
      const fromPinName = Object.keys(fromBlock.Outputs)[this.state.source[1]];
      const toBlock = this.getBlockbyId(nodes, nid);
      const toPinName = Object.keys(toBlock.Inputs)[inputIndex];
  ...
  render() {
    ...
    if (dragging) {
      ...
      const out_name = Object.keys(sourceBlock.Outputs)[this.state.source[1]];
