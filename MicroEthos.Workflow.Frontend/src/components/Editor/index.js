...
export default class Editor extends Component {
    ...
    async componentDidMount() {
        ...
        while (loading) {
            await WorkflowApi.get('modules')
                .then(response => {
                    this.setState({
                        ...this.state,
                        plugins_dict: response.Data,
                        view_mode: VIEW_MODE.GRAPH
                    })
                    loading = false;
                })
                .catch(handleError);
            // await WorkflowApi.get({ id: node_id})
            // .then(loadNode)
            // .catch(handleError);
            if (loading) {
    ...
    checkNodeStatus() {
        ...
        WorkflowApi.endpoints[self.props.collection].getOne({id: node_id})
            .then((response) => {
    ...
    postNode({node, reloadOption, action, retryOnAuth = true} = {}) {
        ...
        WorkflowApi.endpoints[self.props.collection]
            .create({
                node: node,
                action: action
            })
